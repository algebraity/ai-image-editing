...
ACTION_TOOL_SPECS: dict[str, ActionToolSpec] = {
    ...
    for item in [
        ...
        spec(
            ...
            "selection",
            "Create a selection from pixels close to a color.",
            param_schema(
                {
                    "name": string_field("Mask name."),
                    "color": COLOR,
                    "tolerance": number_field("Color distance tolerance.", minimum=0),
                    "bbox_xyxy": BBOX,
        ...
        spec(
            ...
            "selection",
            "Create a contiguous color selection from seed points.",
            param_schema(
                {
                    "name": string_field("Mask name."),
                    "seed_points": POINTS,
                ...
                required=("seed_points", "tolerance"),
            ),
            target_fields={"layer_id": TF.DEFAULT_ACTIVE_LAYER, "mask_id": TF.GENERATED},
        ),
        spec(
        ...
        spec(
            ...
            "mask",
            "Threshold, grow, shrink, feather, or clean a mask.",
            param_schema(
                {
        ...
        spec(ActionType.COLORIZE, "color", "Colorize pixels while preserving luminance.", target_fields={"layer_id": TF.DEFAULT_ACTIVE_LAYER}, write_mask=TF.GENERATED),
        ...
        spec(ActionType.EXTRACT_LINE_ART, "perception", "Create an edge-derived line-art mask.", target_fields={"layer_id": TF.DEFAULT_ACTIVE_LAYER, "mask_id": TF.GENERATED}),
...
_COLOR_PARAMS = param_schema(
    {
        ...
        "source_color": COLOR,
        "target_color": COLOR,
        "tolerance": number_field("Color tolerance."),
        "softness": number_field("Replacement softness."),
        "method": enum_field(("luminance", "average", "lightness"), "Desaturation method."),
        "amount": number_field("Adjustment amount."),
    }
)
...
_PERCEPTION_PARAMS = param_schema(
    {
        ...
        "seed_points": POINTS,
        "mode": enum_field(("alpha", "luminance", "color", "edges"), "Perception mode."),
        "name": string_field("Generated mask name."),
...
for _name in (
    ...
    ActionType.EXTRACT_LINE_ART.value,
